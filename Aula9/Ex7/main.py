...
def main():
    ...
    for idx, normal in enumerate(point_cloud.normals):
        ...
        if abs(angle - 90) < angle_tolerance: # this point has a "vertical normal"
            horizontal_idxs.append(idx)

    vertical_cloud = point_cloud.select_by_index(horizontal_idxs)
    non_vertical_cloud = point_cloud.select_by_index(horizontal_idxs, invert=True)
